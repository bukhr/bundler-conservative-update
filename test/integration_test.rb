...
class IntegrationTest < Minitest::Test
  ...
  def test_update_of_a_named_gem_is_re_executed_once_with_conservative
    in_bundle_project(gems: ["rake"]) do |gemfile|
      output = bundle(gemfile, "update", "rake")

      assert_equal 1, output.scan(PLUGIN_LINE).length, output
    end
  end

  def test_bare_update_is_left_alone
  ...
  def in_bundle_project(gems: [])
    Dir.mktmpdir("bundler-conservative-update") do |dir|
      ...
      File.write(gemfile, <<~GEMFILE)
        ...
        #{gems.map { |name| "gem #{name.inspect}" }.join("\n")}
      GEMFILE
