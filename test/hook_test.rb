...
class HookTest < Minitest::Test
  ...
  def test_does_not_inject_on_bare_update
    hook = Hook.new(argv: ["update"], env: {})

    refute_predicate hook, :should_inject?
  end

  def test_does_not_inject_on_bare_update_with_only_scopeless_flags
    hook = Hook.new(argv: ["update", "--local", "--quiet"], env: {})

    refute_predicate hook, :should_inject?
  end

  def test_does_not_inject_when_the_only_argument_is_a_flag_value
    hook = Hook.new(argv: ["update", "--jobs", "4"], env: {})

    refute_predicate hook, :should_inject?
  end

  def test_injects_with_the_group_flag
    hook = Hook.new(argv: ["update", "--group", "development"], env: {})

    assert_predicate hook, :should_inject?
  end

  def test_injects_with_the_group_flag_carrying_a_value
    hook = Hook.new(argv: ["update", "--group=development"], env: {})

    assert_predicate hook, :should_inject?
  end

  def test_injects_with_the_short_group_flag
    hook = Hook.new(argv: ["update", "-g", "development"], env: {})

    assert_predicate hook, :should_inject?
  end

  def test_injects_with_a_gem_after_a_flag_that_takes_a_value
    hook = Hook.new(argv: ["update", "--jobs", "4", "rails"], env: {})
